...
TirePacejka::TirePacejka(const TireConfig& config, bool isDriven)
...
void TirePacejka::calculate(float verticalLoad, float slipAngle, float slipRatio) {
    ...
    // --- Cornering stiffness
    float Ky = FNOMIN * PKY1 * sin(2.0 * atan(Fz / (PKY2 * FNOMIN))) *
               (1.0 - PKY3 * std::fabs(gamma));

    // --- Stiffness factor
    float By = Ky / (Cy * Dy);

    // --- Curvature factor
    float Ey = (PEY1 + PEY2 * dfz) *
               (1.0 - (PEY3 + PEY4 * gamma) * sgn(ay));
